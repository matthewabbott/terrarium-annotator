...
Same words change referents and same referents change names, both mid-story. **Observed in the t1–40 DB**: `Husk` — a temple-guardian creature, evidenced by GM combat readouts (posts 30855895, 30856881). **Expected, per Matt's knowledge of later threads** (hypotheses to reconcile when reached, not evidence in this DB): a soul-layer also called "husk" (→ split `Husk (creature)` / `Husk (soul layer)`); the Emperor's Hand signet becoming the ring of sanity (enchanted later — identity is temporal); projection sword → shockwave sword (rename); Burnout → Skull Kid (character introduced under an alias). Rules: (1) never auto-merge; (2) split via disambiguation suffixes when a homograph collision is confirmed *in evidence*; (3) rename/alias evolution is expected and cheap (`rename_entry` exists); (4) the researcher's dedupe pass must be temporal-aware: "same in thread 12" ≠ "same in thread 200".
