...
- §1 critic: **TMS/ATMS** (Doyle 1979, de Kleer 1986) — entries' evidence quotes *are* justifications; canon drift withdraws support → retraction with dependency tracking. **CoVe-factored verification** (critic answers "is this claim supported" *without* seeing the draft rationale — avoids self-agreement). **FActScore/RAGAS** claim decomposition. **Self-Refine's documented self-bias** is why the critic gets the author's transcript + future story state rather than re-judging blind.
- §2 epistemics: **Wikidata rank/qualifier/reference** (deprecated-not-deleted, machine-readable retraction reasons); **FactBank** per-source factuality (narrator vs. character vs. inference is literally its annotation scheme); **AGM** epistemic entrenchment (rumor retracts before narrated fact).
- §3 salience: **Generative Agents** recency × importance × relevance; **Mem0 floored decay** (demote-not-delete with a 0.3× floor — dormant terms resurface); **SWAT** co-occurrence-graph centrality (Vys vs Well Flower falls out of graph position); **C-value/contrastive** scoring for story-distinctiveness.
- §4 deferral: **NIL-linking with clustering** (TAC/NILK/"Learn to Not Link") — the "don't create an entry yet" decision is a studied problem with the exact Missing-Entity vs Non-Entity-Phrase distinction we need; **Angell et al. cluster-linking** — second co-referring mention crosses the threshold (the brand-split rule); **CESI** canonicalization for retitle/merge; **Wikidata "structural need"** criterion for when a sub-entry is earned.
